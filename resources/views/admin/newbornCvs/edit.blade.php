...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.newbornCv.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.newborn-cvs.update", [$newbornCv->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="trial_code">{{ trans('cruds.newbornCv.fields.trial_code') }}</label>
                <input class="form-control {{ $errors->has('trial_code') ? 'is-invalid' : '' }}" type="text" name="trial_code" id="trial_code" value="{{ old('trial_code', $newbornCv->trial_code) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('data_type') ? 'is-invalid' : '' }}" name="data_type" id="data_type" required>
                    ...
                    @foreach(App\Models\NewbornCv::DATA_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('data_type', $newbornCv->data_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" type="text" name="notes" id="notes" value="{{ old('notes', $newbornCv->notes) }}">
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
